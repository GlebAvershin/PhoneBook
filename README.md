...
│   Источники событий                  │
├─────────────────────────────────────┤
│ • Web пиксели  (/view/*, /web/*)    │
│ • JS-коллектор (/web/collect)       │
│ • Mobile SDK   (/app/*)             │
│ • Batch API    (/api/*/batch)       │
│ • Call tracking (/calltraking/*)    │
└──────────────────┬──────────────────┘
                   │
        ┌──────────▼──────────┐
        │  Fiber Web Server   │
   ...
   │               │               │
   ▼               ▼               ▼
Handler/Event  Handler/API    Handler/App
   │               │               │
   └───────────────┼───────────────┘
                   │
        ┌──────────▼──────────┐
    ...
    │                             │
    ▼                             ▼
Kafka Topics              Redis Storage
 (async send)         (LRU cache + Sentinel)
 • event_log          • User ID mapping
 • impression_event   • Redirect links
 • sync (IDs)         • VPAID templates
```
